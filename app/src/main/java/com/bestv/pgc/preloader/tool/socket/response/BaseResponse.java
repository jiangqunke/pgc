package com.bestv.pgc.preloader.tool.socket.response;

import android.text.TextUtils;


import com.bestv.pgc.preloader.tool.VideoProxyCacheManager;
import com.bestv.pgc.preloader.tool.common.VideoCacheException;
import com.bestv.pgc.preloader.tool.socket.request.ChunkedOutputStream;
import com.bestv.pgc.preloader.tool.socket.request.ContentType;
import com.bestv.pgc.preloader.tool.socket.request.HttpRequest;
import com.bestv.pgc.preloader.tool.socket.request.IState;
import com.bestv.pgc.preloader.tool.socket.request.Method;
import com.bestv.pgc.preloader.tool.socket.request.ResponseState;
...
public abstract class BaseResponse {
    ...
    protected boolean shouldSendResponse(Socket socket, String md5) {
        ...
        if (socketClosed) {
            LogUtils.d(TAG, "shouldSendResponse socket closed");
        }
        String playingUrlMd5 = VideoProxyCacheManager.getInstance().getPlayingUrlMd5();
        if (!TextUtils.equals(md5, playingUrlMd5)) {
            LogUtils.d(TAG, "url mismatch md5 =" + md5 + " playingUrlMd5 =" + playingUrlMd5);
            LogUtils.d(TAG, "mVideoUrl = " + mVideoUrl);
        }
