...
import com.bestv.pgc.preloader.tool.common.VideoParams;
...
import com.bestv.pgc.preloader.tool.utils.VideoParamsUtils;
...
public class VideoInfoParseManager {
    ...
    public void parseVideoInfo(VideoCacheInfo cacheInfo, Map<String, String> headers,
        ...
        mFuture = VideoProxyThreadUtils.submitRunnableTask(() -> {
            if (!mIsRunning) {
                handleInterrupted(cacheInfo);
                return;
            }
            ...
            LogUtils.d(TAG, "parseVideoInfo url:" + cacheInfo.getVideoUrl() + " parse duration = " + parseDuration);
            mIsRunning = false;
        });
    }
    ...
    private void handleInterrupted(VideoCacheInfo cacheInfo) {
        ...
        if (isM3u8)
            mListener.onM3U8ParsedFailed(new VideoCacheException("m3u8 parse interrupted by user"), cacheInfo);
        else
            mListener.onNonM3U8ParsedFailed(new VideoCacheException("non m3u8 parse interrupted by user"), cacheInfo);
    }
