package com.bestv.pgc.preloader.tool;
...
import com.bestv.pgc.preloader.tool.common.ProxyMessage;
import com.bestv.pgc.preloader.tool.common.VideoCacheConfig;
import com.bestv.pgc.preloader.tool.common.VideoCacheException;
import com.bestv.pgc.preloader.tool.common.VideoType;
import com.bestv.pgc.preloader.tool.listener.IVideoCacheListener;
import com.bestv.pgc.preloader.tool.listener.IVideoCacheTaskListener;
import com.bestv.pgc.preloader.tool.listener.IVideoInfoParsedListener;
import com.bestv.pgc.preloader.tool.listener.VideoInfoParsedListener;
import com.bestv.pgc.preloader.tool.m3u8.M3U8;
import com.bestv.pgc.preloader.tool.model.VideoCacheInfo;
import com.bestv.pgc.preloader.tool.okhttp.IHttpPipelineListener;
import com.bestv.pgc.preloader.tool.okhttp.NetworkConfig;
import com.bestv.pgc.preloader.tool.okhttp.OkHttpManager;
import com.bestv.pgc.preloader.tool.proxy.LocalProxyVideoServer;
import com.bestv.pgc.preloader.tool.task.M3U8CacheTask;
import com.bestv.pgc.preloader.tool.task.Mp4CacheTask;
import com.bestv.pgc.preloader.tool.task.VideoCacheTask;
import com.bestv.pgc.preloader.tool.utils.LogUtils;
import com.bestv.pgc.preloader.tool.utils.ProxyCacheUtils;
import com.bestv.pgc.preloader.tool.utils.StorageUtils;
import com.bestv.pgc.preloader.tool.utils.VideoProxyThreadUtils;
...
public class VideoProxyCacheManager {
    ...
    public void startRequestVideoInfo(String videoUrl, Map<String, String> headers, Map<String, Object> extraParams) {
        //StorageManager.getInstance().initCacheInfo(); // no need to compute storage capacity each request: by rayman
        mStarted = true;
        String md5 = ProxyCacheUtils.computeMD5(videoUrl);
        ...
        if (videoCacheInfo == null) {
            ...
            if (videoCacheInfo.getVideoType() == VideoType.M3U8_TYPE) {
                ...
                if (!mStarted) {
                    mProxyHandler.obtainMessage(ProxyMessage.MSG_VIDEO_PROXY_ERROR, videoCacheInfo).sendToTarget();
                    return;
                }
    ...
    public void stopRequestVideoInfo() {
        mStarted = false;
        VideoInfoParseManager.getInstance().cancel();
    }
    ...
    private void addVideoSeekInfo(String url) {
        ...
        synchronized (mSeekPositionLock) {
            LogUtils.i(TAG, "++++++ addVideoSeekInfo md5=" + md5 + ", url=" + url + " position = -1");
            ...
            if (videoType == VideoType.OTHER_TYPE && mVideoSeekMd5PositionMap.containsKey(md5)) {
                ...
                if (position > 0) {
                    ...
                    if (isMp4PositionSegExisted) {
                        ...
                        LogUtils.i(TAG, "shouldNotifyLock position=" + position + ", url=" + url + " 说明发生了seek, 但是seek请求并没有结束 ");
                    ...
                    LogUtils.i(TAG, "shouldNotifyLock position=" + position + ", url=" + url + " 说明次数有seek操作,但是seek操作还没有从local server端发送过来 ");
