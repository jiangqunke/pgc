package com.bestv.pgc.preloader.tool.task;


import com.bestv.pgc.preloader.tool.common.VideoCacheException;
import com.bestv.pgc.preloader.tool.listener.IMp4CacheThreadListener;
import com.bestv.pgc.preloader.tool.model.VideoRange;
...
public class Mp4VideoCacheThread implements Runnable {
    ...
    private void downloadVideoByOkHttp() {
        ...
        try {
            ...
            inputStream = OkHttpManager.getInstance().getResponseBody(mVideoUrl, mHeaders, contentLength -> {
                ...
                if (mTotalSize != contentLength) {
                    LogUtils.w(TAG, "parsed total length: " + mTotalSize + " is not equal to response body length: " + contentLength);
                    mTotalSize = contentLength;
                    notifyOnCacheTargetSizeUpdate(mTotalSize);
                }
            });
    ...
    private void notifyOnCacheTargetSizeUpdate(long totalSize) {
        mListener.onCacheTargetSizeUpdate(totalSize);
    }
