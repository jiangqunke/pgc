package com.bestv.pgc.preloader.ui.util;
...
import com.bestv.pgc.preloader.ui.PlayerEnvironment;
...
public class AndroidUtils {
    ...
    public static boolean hasEnoughCache(Context context, FileNameGenerator generator, String url, long preloadLengthInBytes) {
        try {
            ...
            if (file.exists() && file.canRead() && file.length() > preloadLengthInBytes) {
                return true;
            }
        } catch (Throwable e) {
        }
    ...
    public static boolean removeCache(Context context, FileNameGenerator generator, String url) {
        try {
            File cacheRoot = StorageUtils.getIndividualCacheDirectory(context);
            String path = cacheRoot.getAbsolutePath();
            ...
            File tempFile = new File(path, name + TEMP_POSTFIX);
            if (tempFile.exists()) {
                tempFile.delete();
            }
