...
import com.bestv.pgc.preloader.ui.adapter.DefaultNetworkAdapter;
        ...
        public void run() {
            ...
            try {
                while (!isInterrupted()) {
                    ...
                    int lastIndex = Math.min(mCurrentIndex + 5, mUrls.size() - 1);
                    ...
                    for (int i = firstIndex; i <= lastIndex; i++) {
                        ...
                        if (!mLoadingTaskDeque.contains(preLoadTask)) {
                            if (mLoadingTaskDeque.size() >= 16) {
                                ...
                                Log.d(PreLoadManager.TAG, "mLoadingTaskDeque size more than 16, remove index: " + ingPreLoadTask.getIndex());
                            }

                            Log.d(PreLoadManager.TAG, "Put into mLoadingTaskDeque: " + preLoadTask.getUrl());
